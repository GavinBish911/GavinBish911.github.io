...
class TaskTest {
	/*
	 * The following tests exercise the Task class. The first 3 tests make sure the
	 * field does not become longer than the constraint (10 characters for task ID,
	 * 20 characters for task name, and 50 characters for task desciption. The last
	 * 2 tests ensure that each field is not null. TaskID is not tested for being
	 * not null because there isn't a way to create a contact with a null taskID.
	 * Likewise, it is not tested for being non-updateable because there is no way
	 * to update it.
	 */
	...
	void testTaskDescWithMoreThanFiftyCharacters() {
		Task task = new Task("Name", "123456789 is nine characters long" + "123456789 is another nine characters long"
				+ "123456789 is another nine characters long" + "123456789 is another nine characters long");
